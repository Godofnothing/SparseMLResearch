...
class SPDY:
    ...
    def __init__(
        self,
        target, db, errors, baselinetime, timings,
        ...
        self.prunabletime = sum(max(c) for c in self.timings)
    ...
    def search(
        ...
        for resamplings in range(int(layerperc * len(self.layers)), 0, -1):
...
if __name__ == '__main__':
    ...
    get_model, test, run = get_functions('deit-tiny')
    dataloader, testloader = get_loaders('imagenet')

    model = get_model()
    DATABASE_DIR = 'deit-tiny_unstr'
    db = UnstrDatabase(DATABASE_DIR, model)
    ERROR_FILE = 'deit-tiny_squared.txt'
    errors = db.load_errors(ERROR_FILE)
    baselinetime, timings = db.get_params(find_layers(model))
    ...
    PROFILE_FILE = 'deit-tiny_%dx.txt' % int(TARGET * 100)
